...
-- adds columns only when missing (for upgrades from older DDL).
--
...
BEGIN
  ...
  SELECT COUNT(*) INTO c FROM user_tab_columns
   WHERE table_name = 'MONITORING_PROJECTS' AND column_name = 'IS_DELETED';
  IF c = 0 THEN
    EXECUTE IMMEDIATE
      q'[ALTER TABLE MONITORING_PROJECTS ADD (IS_DELETED NUMBER(1) DEFAULT 0 NOT NULL)]';
  END IF;
END;
/
